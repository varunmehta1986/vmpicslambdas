...
WORKDIR /src
COPY ["GalleryJsonLambdaFunction.csproj", "GalleryJsonLambdaFunction/"]
RUN dotnet restore "GalleryJsonLambdaFunction/GalleryJsonLambdaFunction.csproj"

WORKDIR "/src/GalleryJsonLambdaFunction"
COPY . .
RUN dotnet build "GalleryJsonLambdaFunction.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "GalleryJsonLambdaFunction.csproj" \
...
Execute unit tests
```
cd "GalleryJsonLambdaFunction/test/GalleryJsonLambdaFunction.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "GalleryJsonLambdaFunction/src/GalleryJsonLambdaFunction"
