...
WORKDIR /src
COPY ["VMPicsLambda.csproj", "VMPicsLambda/"]
RUN dotnet restore "VMPicsLambda/VMPicsLambda.csproj"

WORKDIR "/src/VMPicsLambda"
COPY . .
RUN dotnet build "VMPicsLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "VMPicsLambda.csproj" \
...
Execute unit tests
```
cd "VMPicsLambda/test/VMPicsLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "VMPicsLambda/src/VMPicsLambda"
